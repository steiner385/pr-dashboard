...
export const METRIC_DEFINITIONS = {
  ...
  costActualsAttributed: { label: 'attributed',
    text: 'how much of the actual bill the dashboard can pin to specific tracked CI jobs (job minutes × pool $/min) — a bottom-up reconstruction. It is the ONLY figure that can be sliced per-PR, per-pool, or per-repo, so it powers every “this PR cost $X” number. Always ≤ actual; null until pool rates are set' },
  costActualsCoverage: { label: 'attribution coverage',
    text: 'attributed ÷ actual — a confidence gauge on the breakdown, NOT a dollar amount. ~100% means priced jobs explain virtually the whole bill, so the per-PR/per-pool costs are trustworthy; lower means more of the bill is spend no single job owns (idle runner capacity, node boot/teardown, control-plane, unpriced pools). The headline is the cumulative window ratio (stable — ~90% for fleet, the ~10% gap being non-compute EC2-Other/EKS/VPC). The per-day column is noisy by nature: a blended $/min can’t match each day’s provider cost allocation, so days swing well above and below 100% — read the cumulative, not a single day' },
